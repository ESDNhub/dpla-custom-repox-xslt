...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <mods xmlns="http://www.loc.gov/mods/v3" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
      ...
      <physicalDescription>
        <xsl:apply-templates select="dc:format" mode="aaac"/>
      </physicalDescription>
      ...
      <xsl:apply-templates select="dc:identifier" mode="aaac"/>
      ...
      <xsl:element name="relatedItem" namespace="http://www.loc.gov/mods/v3">
        ...
        <xsl:element name="titleInfo" namespace="http://www.loc.gov/mods/v3">
          <xsl:element name="title" namespace="http://www.loc.gov/mods/v3">Asian American Arts Centre-artasiamerica</xsl:element>
        </xsl:element>
      </xsl:element>

      <xsl:call-template name="owner-note">
        <xsl:with-param name="owner">Asian American Arts Centre</xsl:with-param>
      </xsl:call-template>
  ...
  <xsl:template match="dc:identifier" mode="aaac">
    <xsl:variable name="idvalue" select="normalize-space(.)"/>
    <xsl:if test="normalize-space(.)!='' and not(contains(.,'waystation'))">
      <xsl:choose>
        <xsl:when test="(contains(., 'documents'))">
