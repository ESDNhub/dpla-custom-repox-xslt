...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:oai_dc='http://www.openarchives.org/OAI/2.0/oai_dc/' xmlns:dc="http://purl.org/dc/elements/1.1/" version="2.0" xmlns="http://www.loc.gov/mods/v3">
  ...
  <xsl:template match="//oai_dc:dc">
    <mods xmlns="http://www.loc.gov/mods/v3" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.loc.gov/mods/v3 http://www.loc.gov/standards/mods/v3/mods-3-4.xsd" version="3.4">      
      ...
      <xsl:call-template name="owner-note"><xsl:with-param name="owner">Alfred University</xsl:with-param></xsl:call-template>
...
<xsl:include href="esdn_templates.xsl"/>
  ...
  <xsl:template match="dc:creator" mode="alfred">
  ...
  <xsl:template match="dc:source" mode="alfred">
    <xsl:variable name="quote_delim" select="tokenize(., ';')"/> 
    <extent><xsl:value-of select="normalize-space($quote_delim[2])"/></extent>
    <form><xsl:value-of select="$quote_delim[1]"/></form>
  </xsl:template>
