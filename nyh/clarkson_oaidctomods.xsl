...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <mods xmlns="http://www.loc.gov/mods/v3" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
      ...
      <xsl:if test="dc:publisher != '' or dc:date != ''">
        <originInfo>
          <xsl:apply-templates select="dc:date[lower-case(./text())!='no date'][1]"/>
      ...
      <xsl:if test="normalize-space(dc:format) != ''">
        <physicalDescription>
          <xsl:apply-templates select="dc:format[lower-case(./text()) != 'image/jpeg']" mode="clarkson"/>
        </physicalDescription>
      </xsl:if>
      ...
      <xsl:apply-templates select="dc:language" mode="clarkson"/>
      ...
      <xsl:element name="relatedItem" namespace="http://www.loc.gov/mods/v3">
        ...
        <xsl:element name="titleInfo" namespace="http://www.loc.gov/mods/v3">
          <xsl:element name="title" namespace="http://www.loc.gov/mods/v3">Clarkson University</xsl:element>
        </xsl:element>
      </xsl:element>
      ...
      <xsl:call-template name="owner-note">
        <xsl:with-param name="owner">
          Clarkson University</xsl:with-param>
      </xsl:call-template>
  ...
  <xsl:template match="dc:format" mode="clarkson">
    <xsl:choose>
  ...
  <xsl:template match="dc:language" mode="clarkson">
    <xsl:choose>
