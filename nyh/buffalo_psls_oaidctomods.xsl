...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <mods xmlns="http://www.loc.gov/mods/v3" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
      ...
      <xsl:element name="physicalDescription" namespace="http://www.loc.gov/mods/v3">
        <xsl:apply-templates select="dc:format" mode="buffafo_psls"/>
      </xsl:element>
      
      <xsl:apply-templates select="dc:coverage" mode="esdn"/>
      <xsl:apply-templates select="dc:relation" mode="esdn"/>
      <xsl:apply-templates select="dc:type" mode="buffafo_psls"/>
      ...
      <xsl:element name="relatedItem" namespace="http://www.loc.gov/mods/v3">
        ...
        <xsl:element name="titleInfo" namespace="http://www.loc.gov/mods/v3">
          <xsl:element name="title" namespace="http://www.loc.gov/mods/v3">Buffalo Public School Library System</xsl:element>
        </xsl:element>
      </xsl:element>
      ...
      <xsl:call-template name="owner-note">
        <xsl:with-param name="owner">Buffalo Public School Library System</xsl:with-param>
      </xsl:call-template>
    </mods>
  </xsl:template>
  ...
  <xsl:template match="dc:format" mode="buffafo_psls">
    <xsl:element name="form" namespace="http://www.loc.gov/mods/v3">
      <xsl:value-of select="normalize-space(.)"/>
    </xsl:element>
  </xsl:template>
  
  <xsl:template match="dc:type" mode="buffafo_psls"/>
