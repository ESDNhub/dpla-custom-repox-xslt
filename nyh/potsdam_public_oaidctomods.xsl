...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <mods xmlns="http://www.loc.gov/mods/v3" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
      ...
      <xsl:if test="normalize-space(dc:format) != '' or normalize-space(dc:source) != ''">
        <physicalDescription>
          <xsl:apply-templates select="dc:source" mode="potsdam"/>
          <xsl:apply-templates select="dc:format[lower-case(./text()) != 'image/jpeg']" mode="potsdam"/>
        </physicalDescription>
      </xsl:if>
      ...
      <xsl:element name="relatedItem" namespace="http://www.loc.gov/mods/v3">
        ...
        <xsl:element name="titleInfo" namespace="http://www.loc.gov/mods/v3">
          <xsl:element name="title" namespace="http://www.loc.gov/mods/v3">Potsdam Public
            Museum</xsl:element>
        </xsl:element>
      </xsl:element>
      ...
      <xsl:call-template name="owner-note">
        <xsl:with-param name="owner"> Potsdam Public Museum</xsl:with-param>
      </xsl:call-template>
  ...
  <xsl:template match="dc:format" mode="potsdam">
    <form>
      <xsl:value-of select="normalize-space(.)"/>
    </form>
  </xsl:template>
