...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <mods xmlns="http://www.loc.gov/mods/v3" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
      ...
      <xsl:apply-templates select="dc:source" mode="northville"/>
      ...
      <xsl:element name="relatedItem" namespace="http://www.loc.gov/mods/v3">
        ...
        <xsl:element name="titleInfo" namespace="http://www.loc.gov/mods/v3">
          <xsl:element name="title" namespace="http://www.loc.gov/mods/v3">Northville Public Library</xsl:element>
        </xsl:element>
      </xsl:element>
  ...
  <xsl:template match="dc:source" mode="northville">
    <xsl:variable name="src_list" select="tokenize(., ';')"/>
    <xsl:element name="physicalDescription">
      <xsl:element name="extent">
        <xsl:value-of select="normalize-space($src_list[3])"/>
      </xsl:element>
      <xsl:element name="form">
        <xsl:value-of select="normalize-space(substring-before(., concat(';', $src_list[3])))"/>
      </xsl:element>
    </xsl:element>
